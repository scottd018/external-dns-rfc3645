...
type rfc3645Stub struct {
	output     []*dns.Envelope
	updateMsgs []*dns.Msg
	createMsgs []*dns.Msg
}

func newRfc3645Stub() *rfc3645Stub {
	return &rfc3645Stub{
...
func (r *rfc3645Stub) SendMessage(msg *dns.Msg) error {
	log.Info(msg.String())
	lines := extractRfc3645AuthoritySectionFromMessage(msg)
...
func (r *rfc3645Stub) setOutput(output []string) error {
...
func (r *rfc3645Stub) IncomeTransfer(m *dns.Msg, a string) (env chan *dns.Envelope, err error) {
...
func createRfc3645StubProvider(stub *rfc3645Stub) (Provider, error) {
	return NewRfc3645Provider("", 0, "", "", "", "", "", true, endpoint.DomainFilter{}, false, 300*time.Second, stub)
}

func extractRfc3645AuthoritySectionFromMessage(msg fmt.Stringer) []string {
...
// TestRfc3645GetRecordsMultipleTargets simulates a single record with multiple targets.
func TestRfc3645GetRecordsMultipleTargets(t *testing.T) {
	stub := newRfc3645Stub()
	...
	provider, err := createRfc3645StubProvider(stub)
	assert.NoError(t, err)
...
func TestRfc3645GetRecords(t *testing.T) {
	stub := newRfc3645Stub()
	err := stub.setOutput([]string{
...
func TestRfc3645ApplyChanges(t *testing.T) {
	stub := newRfc3645Stub()
	provider, err := createRfc3645StubProvider(stub)
	assert.NoError(t, err)
...
func TestRfc3645ApplyChangesWithDifferentTTLs(t *testing.T) {
	stub := newRfc3645Stub()
	...
	createRecords := extractRfc3645AuthoritySectionFromMessage(stub.createMsgs[0])
