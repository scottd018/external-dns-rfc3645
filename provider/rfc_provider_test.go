...
func (r *rfcStub) SendMessage(msg *dns.Msg) error {
	log.Info(msg.String())
	lines := extractRfcAuthoritySectionFromMessage(msg)
...
func (r *rfcStub) setOutput(output []string) error {
...
func (r *rfcStub) IncomeTransfer(m *dns.Msg, a string) (env chan *dns.Envelope, err error) {
...
func extractRfcAuthoritySectionFromMessage(msg fmt.Stringer) []string {
...
func TestRfcGetRecordsMultipleTargets(t *testing.T) {
	stub := newRfcStub()
	...
	rfc3645Provider, err := createRfc3645StubProvider(stub)
	assert.NoError(t, err)

	tc = append(tc, rfc3645Provider)

	for _, provider := range tc {
...
func TestRfcGetRecords(t *testing.T) {
	stub := newRfcStub()
	err := stub.setOutput([]string{
...
func TestRfc3645ApplyChangesWithDifferentTTLs(t *testing.T) {
	...
	for _, provider := range tc {
		...
		createRecords := extractRfcAuthoritySectionFromMessage(stub.createMsgs[0])
